...
func Create_registryHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/registries%s", cfg.BaseURL, queryString)
...
func CreateCreate_registryTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_registries",
		mcp.WithDescription("Add a new registry"),
		mcp.WithBoolean("validate", mcp.Description("flag to determine whether or not to validate registry/credential at registry add time")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Create_registryHandler(cfg),
	}
}
